...
func (k *Key) Trust() error {
	...
	err = os.MkdirAll(u.HomeDir+"/.ssh", 0700)
	if err != nil {
		log.Printf("mkdir %s error:%s", u.HomeDir+"/.ssh", err)
	}

	err = ioutil.WriteFile(u.HomeDir+"/.ssh/id_rsa.bm.pub", []byte(k.PublicKey), 0644)
	if err != nil {
		log.Printf("write key error:%s", err)
		return err
	}
	f, err := os.OpenFile(u.HomeDir+"/.ssh/authorized_keys", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0600)
	if err != nil {
...
func ReadHostKey(u string) ([]Key, error) {
	...
	files, _ := ioutil.ReadDir(filepath.Join(homedir, ".ssh"))
